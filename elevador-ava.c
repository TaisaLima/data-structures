...
void enqueue(PriorityQueue* queue, int prioridade, int atual, int destino) {
...
QueueItem dequeue(PriorityQueue* queue) {

    QueueItem item;
    item.key =  queue->items[queue->size - 1].key;
    item.prioridade = queue->items[queue->size - 1].prioridade;
    item.origem = queue->items[queue->size - 1].origem;
    item.destino = queue->items[queue->size - 1].destino;
...
int melhor_elevador(Tunel* tunel, int andar) {
    ...
    if ((d1<d2) && (d1<d3) && (d1<d4) && (d1<d5)) return d1; 

    
    if ((d2<d1) && (d2<d3) && (d2<d4) && (d2<d5)) return d2; 

    
    if ((d3<d1) && (d3<d2) && (d3<d4) && (d3<d5)) return d3; 

    
    if ((d4<d2) && (d4<d1) && (d4<d3) && (d4<d5)) return d4; 

    
    if ((d5<d1) && (d5<d2) && (d5<d3) && (d5<d4)) return d5; 
...
Elevador* get_elevador(Tunel* tunel, int andar) {
    ...
    } else if (tunel->e4->andar == andar) {
        return tunel->e4;
    
    } else if (tunel->e5->andar == andar) {
        return tunel->e5;
    }
    return NULL;
}
...
int main() {
    ...
    int reqs[100]; // Fila de prioridade
    int req_count = 0;
    int atual, destino;
    ...
    while (1){
            ...
            while (1) {
               ...
               exibir_predio(tuneis);
                ...
                printf("eeeeeeeekok");
                    if (atual > 300 || destino > 300 || atual < 1 || destino < 0) { printf("Erro: Tente novamente\n"); getchar(); break;}
                                                    

                    int tunel = melhor_tunel(atual, tuneis);
                    Tunel* t = tuneis[tunel];
                    int elevador_num = melhor_elevador(t, atual);
                ...
                printf("eeeeeeeekok");
                    ...
                    if (elevador_num == 1) elevador = t->e1;
                    if (elevador_num == 2)  elevador = t->e2;
                    if (elevador_num == 3)  elevador = t->e3;          
                    if (elevador_num == 4)  elevador = t->e4;           
                    if (elevador_num == 5)  elevador = t->e5;

                
                    gasto = distance(elevador, atual);
                    tempo = (gasto * 2)/ 60;
                    custo = gasto + distance(elevador, destino);

                printf("%d-- --------\n", custo);

                    enqueue(&calls, custo, atual, destino);
                ...
                printf("jjjjjjjjjjjjjjjjk");

                for (int k = req_count; k > 0; k--) { 

                    QueueItem pedido = dequeue(&calls);
